...
function load_json(e) {
    return JSON.parse(fs.readFileSync("build_win/" + e + "-files.json"));
}

var files = JSON.parse(fs.readFileSync("build_win/files.json"));
...
for (var i = 0; i < argv.length; i++) {
    switch (argv[i]) {
        case "redep":
            ...
            break;
        case "clean":
            console.log("Removing all built files in build_win/");
            child_process.execSync("rm build_win/objs/*.o");
            process.exit(0);
            break;
        case "gtk":
...
function find_object_file_name(fn) {
    ...
    return "build_win/objs/" + result.toString(36) + "-" + name + "-" +
        find_file_append() + ".o";
}
...
var build_files = fs.readdirSync("build_win/");
...
function recompute_indiv(obj, file) {
    ...
    fs.writeFileSync(
        "build_win/" + file + "-files.json", JSON.stringify(obj, null, 4));
}
end_flags
